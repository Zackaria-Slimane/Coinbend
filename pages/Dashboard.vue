		...
		<TransitionRoot as="template" :show="sidebarOpen">
			<Dialog as="div" class="relative z-40 lg:hidden" @close="sidebarOpen = false">
				<TransitionChild as="template"
					enter="transition-opacity ease-linear duration-300" enter-from="opacity-0"
						...
						<DialogPanel
							...
							<div class="h-0 flex-1 overflow-y-auto pt-5 pb-4">
								<div class="flex flex-shrink-0 items-center px-4">
									<img class="h-14 w-auto" src="/coinbend-logo.png"
										alt="Coinbend" />
								</div>
									...
									<template v-for="item in navigation" :key="item.name">
										...
										<Disclosure as="div" v-else class="space-y-1"
											...
											<DisclosureButton
												...
												<svg :class="[open ? 'text-gray-400 rotate-90' : 'text-gray-300', 'ml-3 h-5 w-5 flex-shrink-0 transform transition-colors duration-150 ease-in-out group-hover:text-gray-400']"
								...
								<Disclosure as="div" v-else class="space-y-1"
									...
									<DisclosureButton
										...
										<component :is="item.icon"
											...
											aria-hidden="true" />
										<span class="flex-1">{{ item.name }}</span>
										<svg :class="[open ? 'text-sky-400 rotate-90' : 'text-gray-300', 'ml-3 h-5 w-5 flex-shrink-0 transform transition-colors duration-150 ease-in-out group-hover:text-gray-400']"
											viewBox="0 0 20 20" aria-hidden="true">
											<path d="M6 6L14 10L6 14V6Z"
												fill="currentColor" />
										</svg>
									...
									<DisclosurePanel class="space-y-1">
										<DisclosureButton v-for="subItem in item.children"
											:key="subItem.name" as="a" :href="subItem.href"
			...
			<div class=" flex flex-1 overflow-y-scroll">
				<main class="z-0 flex-1 overflow-y-auto focus:outline-none">
					...
					<div class=" py-6 px-4 sm:px-6 lg:px-8">
						<div class="h-full rounded-lg border-2 border-solid border-gray-200">
							<div
								class="border-b-2 flex justify-between align-middle items-center mx-8">
								<div class="my-8 flex gap-2">
									<TargetModal />
									<ExpenseModal />
								</div>
								<h1 class="text-center ml-8 my-8 text-xl">Available to spend :
				...
				<aside
					...
					<div class="absolute inset-0 py-6 px-4 sm:px-6 lg:px-8">
						<div
							class="h-full rounded-lg border-2 border-solid border-gray-200" />
					</div>
